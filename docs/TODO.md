...
- [ ] **`docs/db/mediana_daily_shift_1bizday.sql` 실행** (사용자) — 백업 → DRY-RUN → 트랜잭션 → 검증
  - ⚠ `daily` 만 옮기면 **과거 자금일보 검증식이 전부 깨진다** → `daily_reports.report_date` 동반 시프트 필수
  - 영업일 계산에 공휴일 테이블 불필요 — 기존 행이 영업일에만 있으므로 "다음 행의 날짜"로 밀면 정확
  - UNIQUE(company,date) 충돌 회피를 위해 2단계(임시 오프셋 → 목표) 이동
