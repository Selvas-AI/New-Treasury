...
## 🧭 세션26차 11~13일차 (2026-08-20 완료)

상세: `docs/기획/인수인계_세션26_11-13일차.md` · 설계: `docs/기획/외화원장_계좌간거래_설계.md`

### FX 발의·집행 흐름 복구 (11일차)
- [x] 발의한 매각 지시가 날짜 필터에 걸려 목록에 없던 문제 — 이행 대기 고정 패널 + 딥링크(`?order=`)
- [x] 실무 담당자 매도 발의 경로 신설 — `lib/fxBandExceed.ts` SSOT + 리짐/원장/대시보드 3곳 연결
- [x] 매각 지시 직접 등록(지시일 소급 가능) + 승인 확인 패널(`승인만`/`승인 후 체결 등록`)
- [x] 체결 모달 FIFO 소진 미리보기
- [x] **연계성 우선 개발 규칙 문서화** — CLAUDE.md §1-A (5문항 체크리스트 + FX 워크플로우 연계 지도)

### 외화 원장 재설계 (12일차)
- [x] 원장을 재고 명세형(로트 1건 = 1행)으로 전환 — 중복 표시·자금일보 유출 누락 해소
- [x] FIFO 계좌유형 소진 우선순위(법인별 정책)
- [x] 계좌 간 대체 — 원자적 [FIFO 소진 + 원가승계 신규 로트] 쌍
- [x] 거래 유형(`txn_type`) 분리 — 매입대금 결제가 매각 실적에 섞이던 문제
- [x] 정기예금 해지·재예치 + 운용자금(`investments`) 연동 + 정합성 점검
- [x] 재예치 자기 소진 방어 hotfix
- [x] 대체 로트 가시성(배지·이력 목록·원복 UI)
- [x] 원장 정렬·필터(7개 컬럼 헤더 정렬 + 상태/잔액 필터)

### 권한 UI 통합 (13일차)
- [x] `src/lib/navTree.ts` SSOT — 사이드바와 권한 트리가 같은 상수를 읽는다
- [x] 메뉴 접근 권한을 사이드바와 동일한 트리로 (섹션 3-state·slug 병기·역할 기본값 미리보기)
- [x] 권한 3탭 → 단일 트리 (작업 권한 인라인, 카테고리 권한은 자금일보 행 아래 접이식)
- [x] **`audit-log` 부여 가능해짐** — 기존엔 권한 목록·역할 기본값 양쪽에 없어 master 외 접근 불가였음

### 남은 것 (선택)
- [ ] 필요한 계정에 `audit-log` 권한 부여
- [ ] 기존 정기예금 2건을 운용자금에 매핑 (미매핑 시 정합성 카드에 불일치 경고 지속)
- [ ] `daily`/`history` slug 분리 — 현재는 "함께 열림"으로 표시만. 분리 시 `menus` 마이그레이션 필요
...
### 외화 잔액 원천 불일치 (FX 원장 vs 자금현황)
실측(메디아나 USD): 원장 `fx_lots` 11,088,438 vs 자금현황 `daily+investments` 13,012,683.93 → 차이 1,924,245.93

- [ ] **조정표(Reconciliation Statement) 구현** — 두 값을 억지로 맞추는 게 아니라 **차이를 설명**하는 구조
  - 원장 = **거래 기준**(체결·대체 즉시), 자금현황 = **잔액 확정 기준**(익일 기재) → 실시간 일치는 원리적으로 불가능
  - 은행계정조정표와 같은 구조: `원장잔액 + 미반영 증감 − 미반영 원장거래 = 자금현황잔액`
  - 다리는 이미 있음 — `useFxLedgerReconciliation`("자금일보 미반영 증감")
  - as-of 날짜 기준 원장 잔액 재구성: `acquired_date ≤ D` − `disposed_date ≤ D`
  - **잔차(미설명 차액)가 남을 때만** 대시보드 이슈로 승격 — 상시 경고는 노이즈가 된다

### 메디아나 자금현황 기재일 +1 영업일 시프트
- [x] **`docs/db/mediana_daily_shift_1bizday.sql` 실행** → **2026-08-20 완료**
  - `daily` 87건 `04-14~08-19` → `04-15~08-20` · `daily_reports` 53건 `06-04~08-20` → `06-05~08-21`
  - 검증 완료: 값 이동·광복절 연휴(8/14→8/18)·다른 법인 무영향·중복 0·오프셋 잔존 0 + 실화면 확인
  - 백업 `backup.daily_20260820` / `backup.daily_reports_20260820` — 며칠 유지 후 §5 로 정리
  - ⚠ 구 8/20 초안 1건이 **8/21 로 이동** — 내일 일보 열면 항목 2건이 보인다
- [ ] **`investments`(운용자금) 시프트 여부 확인** — `start_date`/`maturity` 는 잔액 기재일이 아니라
      **계약일**이라 밀면 만기 판정·이자 계산·FIFO 정기예금 잠금이 어긋난다.
      계약일 자체가 늦게 입력돼 왔다면 별도 스크립트 필요, 입력 시점만 늦었다면 손대면 안 된다.
