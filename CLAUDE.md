...
### 2026-08-25 세션27차 (후속) — 운용자금·차입금 입출금액을 레코드에서 도출 ⭐

사용자 확인: "자금일보 운용자금에서 입금·출금으로 표시되지 않고 마감잔액에서만 추가되는 형식이네요."
사용자가 기대한 모델(그리고 옳은 모델):

```
기초잔액 = 전전영업일 마감 · 마감잔액 = 전영업일 마감
그 차이를 보고대상일 거래로 소명한다.
어제 만기해지·재예치가 운용자금에 전일자로 입력돼 있으면
  → 해지액이 운용자금 출금, 재예치액이 운용자금 입금으로 자동으로 불러와지고
  → 두 금액의 차이(이자 등)만 자금일보 입금 항목으로 추가 기재하면 된다.
```

#### [설계 변경] 행별 입출금액을 linked_id 귀속 → **레코드 도출**로 전환
```
직전 커밋(37d3dfc)은 자금일보 항목의 linked_id 로 행을 찾아 붙였다. 그런데 그 링크는
자금일보 연동 팝업으로 등록했을 때만 생긴다 — 운용자금 메뉴에서 직접 등록하면
입금 칸이 비고 마감잔액만 뛰어서 행 단위 항등식이 깨져 보인다(실제 30억 사례).

변경: 입금 = Σ(마감엔 있고 기초엔 없는 건) / 출금 = Σ(기초엔 있고 마감엔 없는 건).
      각 레코드는 기초·마감 중 한쪽 또는 양쪽에만 들어가므로
      `마감 − 기초 = 입금 − 출금` 이 **구조적으로 항상 성립**한다.
      등록 경로(운용자금 메뉴 / 연동 팝업)와 무관하게 채워진다.
      InvestGroup.flowInKrw/flowOutKrw · LoanGroup.flowInKrw/flowOutKrw.

소명 유도: 도출액과 자금일보 항목 합계(investIn/Out)가 다르면 운용자금 소계 아래에
      "개시액 X원 중 자금일보 투자집행으로 소명된 금액은 Y원" 안내를 표시한다.
      → 재예치 원금은 자동으로 맞고, 차액(이자)만 입금 항목으로 넣으면 되는 흐름.
금지: 행의 입출금액을 자금일보 항목(linked_id)만으로 채우지 말 것 — 등록 경로에 따라
      비어버린다. linked_id 귀속은 추적·소명 대조용으로만 남긴다.
```

#### [CRITICAL] closed_date 가 항상 "처리한 날"로 박히던 문제
```
setActive(id,false) 가 closed_date = 오늘 로 고정하고 있었다.
어제 해지된 건을 오늘 처리하면 closed_date=오늘 → 어제를 보고대상일로 하는 일보에서는
그 자금이 아직 살아 있는 것으로 잡혀 **출금이 영원히 소명되지 않는다.**
해결: setActive(id, active, closedDate?) 로 실제 해지·상환일을 받는다(미지정 시 오늘 폴백).
      InvestPage/LoansPage 의 만기·상환처리를 window.confirm → CloseDateModal
      (날짜 입력 + 확인 패널)로 교체. 복원은 날짜 없이 확인만.
      ⚠ window.confirm 은 크롬이 반복 대화상자를 차단하면 즉시 false 를 반환해
        "눌러도 아무 일이 없는" 상태가 된다(세션24차 실사고) — 데이터 변경에는 쓰지 말 것.
```
...
#### 자금일보 결재 — 날짜 표기 + 상신자=결재자 자동 승인 (2026-09-04 사용자 리포트)
```
① 승인/반려 모달이 '9-04 자금일보' 화면에서 '9-03 자금일보를 승인합니다' 로 표시됐다.
   일보의 키는 **작성일(selectedDate)** 인데 모달만 보고대상일(reportDate)을 썼다.
   → '{작성일} 자금일보(보고대상일 {reportDate})' 로 둘 다 표기해 혼동을 없앤다.
② 상신자 = 결재자 자동 승인이 동작하지 않는 경우가 있었다. 원인 두 가지를 함께 정리:
   · 결재선의 approver_code 는 손으로 입력해 'K001 ' / 'k001' 같은 값이 들어온다
     → sameCode() 로 trim + 소문자 비교. myApproveStep 판정도 같은 함수를 쓴다(SSOT).
   · handleSubmit 이 saveReport() 로 방금 만든 일보를 submitReport() 에 알리지 못했다
     (훅 콜백의 클로저가 report=null) → **최초 상신이 조용히 false 로 실패**.
     submitReport/approveReport 에 reportIdOverride 인자를 추가해 생성된 id 를 넘긴다.
③ 자동 승인 범위를 1단계 → **상신자 본인이 결재자인 앞쪽 연속 구간 전체**로 확장.
   다른 사람이 결재자인 단계를 만나면 즉시 멈춘다(남의 검토는 건너뛰지 않는다).
   결재선 전 단계가 본인이면 상신과 동시에 승인 확정된다.
금지: 결재자 코드를 === 로 직접 비교하지 말 것(공백·대소문자로 조용히 어긋난다).
```

---

### 2026-09-15 세션29차 — 외화 정기예금 이중 입력 제거 (운용자금 → 원장 자동 반영) ⭐

사용자 리포트: "외화 정기예금 만기가 되어 **운용자금에서 재예치**로 처리하였는데,
외화(FX)관리의 **정기예금 관리에서도 별도로 해지**를 진행하여야 합니다. 연동으로 처리할 수 있는지."

#### [CRITICAL] 같은 사실을 두 장부에 손으로 두 번 넣고 있었다
```
정기예금은 두 장부에 동시에 존재한다.
  investments — 계약 조건(은행·금리·만기)·만기 처리 여부의 정본
  fx_lots     — 외화 원가(장부환율)·FIFO 순서·환전 가능 여부의 정본
세션26차 12일차에 연결 컬럼(investment_id)과 해지 RPC 는 만들었지만, **실행 경로가
외화거래명세 한 곳뿐**이었다(§1-A 1번 — 진입 경로 점검 누락).
  · 실무자는 운용자금 화면에서만 만기·재예치를 처리한다
  · 외화거래명세(fx-ledger)는 메뉴 권한이 따로라 경로 자체가 없는 계정도 있다
→ 원장에 만기 지난 정기예금이 그대로 남아 **환전 가능액이 과소 계산**되고
  (isLotAvailable 이 term_deposit 을 만기 전까지 잠그므로) 리짐 권고액까지 어긋난다.
  두 장부가 벌어진 사실은 정기예금 관리 카드의 정합성 경고로만 드러났다.

해결: 운용자금 처리 한 번으로 원장까지 반영한다.
  · RolloverModal(연장) / CloseDateModal(만기처리)에 "외화거래명세에도 함께 반영" 체크
    (외화 정기예금일 때만 노출, 기본 ON). 끄면 종전처럼 원장에서 따로 해지.
  · 반영 전 **화면 안 미리보기** — 원금 대체액·이자 로트·중도해지 여부·잔액 부족 경고.
⚠ 두 장부를 한 트랜잭션으로 묶을 수 없다(한쪽은 테이블 쓰기, 한쪽은 RPC).
  **운용자금을 먼저** 처리하고 원장을 뒤에 반영한다 — 원장이 실패해도 운용자금은 화면에서
  되돌릴 수 있고, "연장은 완료됐지만 원장 반영 실패 — 직접 해지하세요"로 정확히 알린다.
  조용히 넘어가면 두 장부가 벌어진 줄 모른다.
```

#### 함께 고친 서버 결함 2건 (`docs/db/fx_term_deposit_investment_link.sql` ⭐⭐ **실행 필요**)
```
① 이자를 정기예금에 **재예치**할 수 없었다
   settle_fx_term_deposit 은 재예치(to='term_deposit')일 때 이자를 **항상 보통예금**으로
   넣었다. 그런데 실무의 재예치는 대부분 이자를 원금에 더해 다시 묶는다(복리).
   그러면 원장 정기예금이 이자만큼 운용자금보다 작아져 정합성 경고가 뜬다.
   → p_interest_account_type 추가(미지정이면 종전 동작 그대로).
   ⚠ 정기예금으로 들어가는 이자에 만기가 없으면 그 로트는 **영원히 잠긴다**
     (환전 가능 판정이 maturity_date <= 기준일 이라 null 은 통과 못 한다) → 서버가 거부한다.
   ⚠ 인자 추가는 create or replace 가 아니라 **새 오버로드**를 만든다 → drop 후 재생성.

② 중도해지 시 **만기 도래분이 뒤로 밀렸다**
   transfer_fx_lots 의 소진 순서가 `order by acquired_date` 뿐이었다. 만기 도래분만
   대상일 때는 필터가 미도래분을 빼주지만, **p_allow_early 를 켜는 순간** 미도래분이
   후보에 들어오고 취득일이 앞서면 그쪽이 먼저 깨진다.
   실측: 메디아나 USD — 2026-03-11 취득(만기 12-11) 1M / 2026-06-17 취득(만기 09-17) 5M.
   → `만기 도래분 우선 → 그 안에서 취득일 FIFO` 로 교정.
```

#### 추정하지 않는다 — 감액 연장은 자동 반영을 거부
연장 금액이 기존보다 **적으면** 줄어든 돈이 어디로 갔는지(보통예금 이체·매각·대외 지급)
시스템은 알 수 없다. 임의로 보통예금 이체로 잡으면 원장이 사실과 달라진다.
→ 계획 단계에서 막고 "외화거래명세에서 직접 기록하세요"로 안내한다.

#### 변경 파일
- `src/lib/fxTermSettle.ts` (신규) + `fxTermSettle.test.ts` (14건) — 운용자금 값 → 원장 해지
  파라미터 변환. **DB 를 모르는 순수 함수**라 테스트로 검증된다. 화면(미리보기)과 저장이
  같은 함수를 쓴다 — 화면에서 다시 계산하면 갈라진다(세션19차 6.2%/27.9% 유형).
- `src/hooks/useFxLots.ts` — 모듈 레벨 `fetchFxLots` / `settleFxTermDeposit` /
  `linkFxLotsToInvestment` 공개. 훅은 이들을 감싸기만 한다(**RPC 인자 조립은 한 곳**).
  ⚠ 운용자금 화면은 행마다 통화가 달라 `useFxLots(company, currency)` 를 쓸 수 없다
    (조건부 훅 호출 금지) — 그래서 조회·실행 경로를 함수로 뺐다.
- `src/hooks/useInvestments.ts` — `rollover` 반환을 `{ error, newId }` 로 변경.
  새 운용자금 건의 id 가 있어야 원장 로트를 거기에 연결할 수 있다.
- `src/components/common/RolloverModal.tsx` — 원장 연동 섹션 + 이자 장부환율 입력.
- `src/components/common/CloseDateModal.tsx` — `extra` 슬롯 + `confirmDisabled` 추가
  (차입금 상환에서도 쓰는 공용 모달이라 **선택 슬롯**으로만 열어 영향 범위를 막았다).
- `src/pages/InvestPage.tsx` — 두 모달에 원장 상태 주입 + 저장 후 원장 반영·연결.
- `src/components/fx/FxTermDepositCard.tsx` — 안내 갱신(운용자금에서 하면 여기까지 반영됨).

검증: tsc -b 0 errors · eslint 0 errors(기존 warning 수준 유지) · vitest --dir src **156/156**
(신규 14건) · vite build 성공. 실화면 검증은 마이그레이션 실행 후 사용자 확인 예정.

---


### 2026-09-15 세션29차 — 지분·국채 동일 기준일 중복 행 누적 (해결) ⭐

사용자 리포트: 지분/장기투자에서 같은 종목의 같은 기준일 행이 누적 저장돼
해당 일자 자산규모가 뻥튀기됨(캡처: 2026-09-08 · 09-10 각 2건, 주가까지 동일).

#### [CRITICAL] 중복 검사가 클라이언트 스냅샷에만 있었다 (TOCTOU)
```
증상: 같은 종목·같은 기준일 행이 2건씩 쌓인다. 실측 73그룹, 전부 건수=2.

부풀려지는 곳은 **날짜별로 합산하는 화면뿐**이다.
  EquityCard.tsx  dateMap.set(h.date, 기존 + h.total_value)  ← 같은 날짜 행을 전부 더한다
→ 추이 차트의 그 날짜만 2배로 솟는다. KPI·자금일보는 getLatestEquities/
  getLatestBonds 가 종목별 1건만 쓰므로 **잔액 자체는 영향이 없었다.**
  (사용자가 "자산규모 뻥튀기"로 인지한 것은 차트다 — 원인 추적 시 KPI 를 먼저
   의심하면 헛짚는다)

원인: 중복 검사가 세 경로 모두 **조회 시점의 클라이언트 스냅샷**에만 있었다.
  · useEquities.save        → data 배열에서 find
  · autoRefreshPrices       → allRows 스냅샷에서 some
  자동 갱신 1회 실행은 **localStorage** 로만 막는다 → 다른 PC·다른 사용자·다른
  브라우저가 같은 날 접속하면 각각 실행되고, 두 실행이 겹치면 둘 다 "없음"을 보고
  둘 다 INSERT 한다. equities 에 유니크 제약이 없어 DB 가 막지 못했다.
  ⚠ 중복 행의 **주가까지 동일**한 것이 이 설명의 근거다(같은 날 같은 GAS 응답).

해결: DB 유니크 인덱스가 근본 차단이다. 클라이언트 검사로는 경합을 막을 수 없다.
  · equities_company_name_date_uniq (company, name, date)
  · investments_bond_company_ticker_date_uniq — **국채만 부분 유니크**
    (비국채는 같은 은행·같은 날 복수 계약이 정상이다)
  · useEquities.save    — 23505 가 오면 기존 행을 찾아 **수정으로 전환**
  · autoRefreshPrices   — 23505 는 skip(경합에서 진 것), 그 외는 fail
금지: 중복 방지를 클라이언트 스냅샷 검사만으로 끝내지 말 것.
```

#### [BUG] autoRefreshPrices 가 restInsert 의 반환 error 를 통째로 무시했다
`await restInsert(...)` 후 결과를 보지 않고 무조건 `equityOk++` 하고 있었다 —
**INSERT 가 실패해도 성공으로 집계돼 왔다.** 이제 `console.warn` 으로 남기고 fail 로 센다.
(세션26차의 "400 이 catch 에 삼켜져 국채 갱신이 죽은 줄도 몰랐다"와 같은 유형)

#### 삭제 규칙 — created_at 으로 종가를 남긴다
```
73그룹 중 33그룹이 "값이 다른 중복"이었다. 전부 같은 날 주가가 다르다
(예: 메디아나 2026-05-28 → 15,610 / 16,400). 장중 한 번, 종가 확정 후 한 번
조회된 것이다. "평가액이 큰 행 보존"은 오르는 날엔 맞고 **내리는 날엔 틀린다.**

⚠ equities 에 created_at(timestamptz)이 **있었다**. 코드의 EquityRecord 는 12개
  필드만 정의해 그 존재를 모르고 있었다(DB 13컬럼). 이것으로 종가를 정확히 고른다.

보존 우선순위: ① 자금일보(linked_id)가 참조하는 행 ② 값이 유효한 행(total_value>0·
  price>0) ③ created_at 이 늦은 행(=종가) ④ 취득가액 입력분 ⑤ 평가액 큰 행 ⑥ id
⚠ ②가 ③보다 앞인 이유: **0원 행이 더 늦게 들어온 사례가 실재한다**
  (셀바스헬스케어 2026-07-01 → price 0 / 2,825). ③만 쓰면 0원을 남기게 된다.
```

#### `docs/db/equity_bond_duplicate_cleanup.sql` — **실행 완료** (2026-09-15)
결과: 지분 **73건 삭제** · 국채 0건(**중복 자체가 없었다**) · 남은 중복 0그룹 ·
유니크 인덱스 2개 생성 완료. 백업 `backup.equities_dup_20260915` / `investments_dup_20260915`.

⚠ 스크립트 작성 중 겪은 것 — **진단 쿼리를 10개로 늘려 사용자가 단계마다 판단해야
하는 구조를 만들었다가 "너무 복잡하다"는 지적을 받았다.** 판단이 끝난 결론은 스크립트
안에 넣고 **한 번 실행으로 끝나게** 할 것(백업→삭제→인덱스→결과표, 멱등).
진단은 주석으로 내려둔다. 조사 과정을 그대로 사용자에게 넘기지 말 것.

⚠ `daily_report_items.linked_id` 는 **uuid**, `equities.id`/`investments.id` 는 **text** 다.
그대로 조인하면 `42883 operator does not exist: uuid = text`. 양쪽을 `::text` 로 맞춘다.

⚠ Supabase SQL Editor 는 여러 문장을 한 번에 실행하면 **마지막 문장의 결과만** 보여준다.
다건 스크립트는 결과를 테이블에 적고 마지막에 그 테이블을 select 할 것.

검증: tsc -b 0 errors · eslint 0 errors · vitest --dir src 156/156.

---


### 2026-09-16 세션29차 — 국채 평가손실이 입금액으로 표시되던 버그 ⭐

사용자 리포트(최새나): "자금일보 자금현황의 비예금성 국채 항목 — 기초잔고 대비
마감잔액이 30,479,828원 **감소**하였으나 해당 금액이 **입금액으로 표시**되고 있습니다."
(국고02625-5503(25-2) · 기초 5,143,470,975 → 마감 5,112,991,147 · Δ −30,479,828)

#### [CRITICAL] 같은 규칙을 두 effect 에 복제 → 한쪽만 고쳐져 갈라졌다
```
국채 행의 입금·출금은 flowInKrw 가 아니라 **평가손익 자동기재 항목**에서 온다
(memo = @auto:bond:{채권명} → byBondLabel 로 그 행에 귀속).

자동기재 effect 가 지분·국채 **두 벌**로 존재하는데, 기존 항목을 갱신하는 경로가
서로 달랐다:
  · 지분 effect  — direction 도 재검증한다("이전 오류로 잘못 저장된 경우 수정")
  · 국채 effect  — **amount 만 갱신**하고 direction/category 는 손대지 않는다

그래서 평가이익(in)으로 만들어진 국채 항목이 이후 기준가 하락으로 평가손실이 돼도
**금액만 바뀌고 방향은 in 으로 굳었다** → 마감잔액은 줄었는데 입금액에 표시.
⚠ 세션14차에 지분 쪽에 direction 재검증을 넣을 때 국채 effect 에는 반영하지 않은 것이
  그대로 남아 있었다(§1-A 4번 SSOT 위반의 전형 — 규칙이 두 곳에 있으면 반드시 갈라진다).

해결: evalFlow(diff) 를 모듈 레벨 **단일 정본**으로 두고 두 effect 가 함께 쓴다.
  국채 effect 의 existing 맵에 direction 을 담고(기존엔 id·amount 만 담아 비교 자체가
  불가능했다) 지분과 동일한 needsUpdate 판정을 적용.
금지: 방향·카테고리 분기를 effect 안에서 다시 쓰지 말 것.
```

⚠ **기존에 잘못 저장된 항목은 해당 일보를 다시 열면 자동 교정된다**(지분과 같은 패턴) —
별도 데이터 정리 스크립트가 필요 없다. DB 마이그레이션도 불필요하다.

⚠ 검증식(`입금−출금−잔액증감=0`)에는 영향이 없다 — `invest_eval_*` 는 opIn/opOut 집계에서
이미 제외돼 있다(현금 이동이 아니므로). 바뀌는 것은 자금현황 표의 표시 위치뿐이다.
...
### 2026-09-18 세션29차 — 예상 만기이자 과대 계상 2건 + 원가승계 안내 ⭐

사용자 리포트(최새나): "운용자금 정기예금 연장처리하면 외화거래명세에서 연동으로
처리되는데 **장부환율은 별도로 변경해야 하는 건가요?** 그리고 **예상 만기이자 금액은
어떻게 산출된 건지** 확인 부탁드립니다."

#### Q1 장부환율 — 정상 동작. 바꾸면 안 된다
```
대체 로트가 원본의 장부환율·취득일을 승계하는 것은 원가승계(carryover) 정책의 의도다.
  · 재예치는 같은 외화가 회사에 계속 남아 있는 것 — 취득원가가 바뀔 사건이 아니다.
  · 장부환율을 시장환율로 바꾸면 **미실현 손실을 임의로 실현**시켜 환차손익이 왜곡된다.
  · 취득일 승계는 FIFO 소진 순서 보존을 위한 것이다(세션26차 12일차).
문제는 코드가 아니라 **화면이 그 사실을 설명하지 않은 것**이었다 → 대체 배지 툴팁에 명시.
```

#### [CRITICAL] Q2 ① 소진된 정기예금이 예상이자에 합산됐다
```
증상: 이미 해지·소진된 정기예금이 "예상 만기이자"에 계속 잡힌다.
원인: expectedTermInterestFx 가 **originalAmount** 를 쓰고 remainingAmount 필터가 없었다.
      같은 줄의 totalAmount·availableAmount 는 remainingAmount 를 쓰는데 이자만 달랐다.
해결: remainingAmount 기준으로 통일(+ remaining<=0 조기 반환).
      중도해지로 일부만 남은 예금도 남은 원금 기준이 맞고, 만기에 이자를 수취해
      이자 로트로 등록하면 그 예금이 소진되며 예상이자에서 자연히 빠진다.
```

#### [CRITICAL] Q2 ② 대체 로트의 이자 기간이 과대 계산됐다 — 취득일 승계의 부작용
```
Q1 의 "취득일 승계"가 여기서 부작용을 냈다. 이자 계산이 승계된 취득일을 예치 시작일로
써서 **이전 예치 기간까지 이자를 계산**했다.
  실측(메디아나 USD): 06-17 취득 승계 / 만기 12-17 → 183일로 계산.
                      실제 재예치 기간은 09-17→12-17 = 91일 (약 2배).
                      03-11 취득 승계 / 만기 12-11 → 275일 vs 실제 91일 (약 3배).
재예치를 반복할수록 오차가 누적된다.

해결: 이자 기산일을 분리했다 — lib/fxLots.interestStartDate(lot, transferDateById).
  · 대체 로트(transferId 있음) → fx_lot_transfers.transfer_date
  · 그 외(개시·수동 유입·자금일보 반영) → acquiredDate (그대로가 맞다)
  useFxLots 가 fx_lot_transfers 를 함께 읽어 transferId→transfer_date 맵을 만든다.
⚠ DB 마이그레이션을 **일부러 피했다.** interest_start_date 컬럼 추가안은
  transfer_fx_lots 를 또 재정의해야 하는데, 그 함수는 9/15 에 "만기 도래분 우선 소진"으로
  막 고친 것이라 연달아 건드리면 덮일 위험이 있다. 클라이언트 조인은 **기존 로트까지
  즉시 소급 교정**되는 이점도 있다(컬럼 방식은 backfill 이 별도로 필요).
⚠ 대체 이력 조회가 실패해도 로트 조회는 살린다 — 맵이 비면 acquiredDate 로 폴백되어
  종전과 같은 값이 나올 뿐 화면이 깨지지 않는다.
금지: 정기예금 이자 계산에 acquiredDate 를 직접 쓰지 말 것 — 반드시 interestStartDate 경유.
```

#### 표시 개선 (재문의를 막는 부분)
- `대체` 배지 툴팁 — 원가승계라 **장부환율을 바꿀 필요가 없다**는 것과 이자 기산일이 다르다는 것
- `예상 만기이자` 카드에 `ⓘ` — 산출식(잔여원금 × 연이율 × 예치일수 ÷ 365, **단리·세전**) 명시
- 원장 정기예금 행에 `(이자 기산 YYYY-MM-DD)` — **유입일과 다를 때만** 표시

검증: tsc -b 0 errors · eslint 0 errors · vitest --dir src **161/161**(신규 5건).
DB 마이그레이션 불필요.

### 2026-09-18 세션29차 (후속) — 회사 회계정책은 재평가(revalue)였다 ⭐

사용자 확인(회계팀): "외화 정기예금 로직은 **실제가입일에 매매기준율 환율로 새롭게 적용**하고
있습니다." 실제 분개(9/17 $5,000,000 재예치):

```
차) 기타단기금융상품 5,000,000 × 1,368.30 = 6,841,500,000   ← 재예치일 매매기준율
차) 환차손                                =   726,000,000
    대) 기타단기금융상품 5,000,000 × 1,513.50 = 7,567,500,000   ← 기존 장부환율 소멸
```

검산 결과 서버 `revalue` 공식과 정확히 일치한다 — (1,368.30 − 1,513.50) × 5,000,000 = −726,000,000.

⚠ **직전 커밋(22dc7de)에서 "장부환율을 바꿀 필요 없다"고 화면에 안내한 것은 이 회사 정책과
  맞지 않았다.** 원가승계(carryover)는 시스템 기본값일 뿐 보편 정답이 아니다. 평가 방식은
  **법인 정책**(`policy_params.fx_transfer_valuation`)이며, 안내 문구가 정책과 무관하게
  한쪽을 단정하면 사용자를 잘못 인도한다.

#### [CRITICAL] 정책만 바꾸면 운용자금 연장이 깨지는 상태였다
```
증상(잠재): 자금정책에서 평가 방식을 revalue 로 바꾸는 순간, 운용자금 › 정기예금
      연장·만기처리의 "외화거래명세에도 함께 반영"이 100% 실패한다.
원인: 서버 transfer_fx_lots 는 revalue 인데 p_transfer_rate 가 없으면 거부한다.
      그런데 InvestPage.applyLedgerSettle 이 transferRate: null 을 하드코딩하고 있었다.
      외화거래명세 › 계좌 간 대체(FxTransferCard)는 처음부터 revalue 를 지원했지만,
      9/15 에 추가한 운용자금 연동 경로만 그 분기를 빠뜨렸다
      (§1-A 1번 — 진입 경로가 둘인데 한쪽만 구현).
      게다가 운용자금이 먼저 저장되므로 실패하면 두 장부가 벌어진 채 남는다.
해결: 법인 정책을 읽어 revalue 면 모달에서 매매기준율을 먼저 받고, 없으면 계획 단계에서
      막는다(저장 버튼 비활성). 서버가 거부하기 전에 화면에서 차단한다.
금지: 두 진입 경로(외화거래명세 / 운용자금) 중 한쪽에만 정책 분기를 구현하지 말 것.
```

#### 저장 전 환차손익 미리보기 — 회계 분개와 대조 가능하게
`previewTermPrincipalConsumption` + `revaluePnlKRW`(순수 함수). 해지 원금이 어느 로트에서
얼마씩 빠지는지와 확정 환차손익을 **저장 전에** 보여준다.
⚠ 소진 순서는 서버 `transfer_fx_lots` 와 **반드시 동일**해야 한다 —
  `만기 도래분 우선 → 취득일 FIFO → id`. 순수 취득일 FIFO 로 미리 보여주면
  중도해지(allowEarly)일 때 화면과 실제 결과가 갈라진다(2026-09-15 서버 교정과 같은 함정).

#### 정책에 따라 정반대가 되는 안내 문구를 정책 인자로 전환
- 원장 `대체` 배지 툴팁 — carryover/revalue 각각의 설명(재평가면 "대체일 환율로 새로 잡혔고
  차액은 그때 확정됐다 · 유입일도 리셋되어 FIFO 후순위")
- 연장·만기처리 모달의 `(장부환율 승계, 손익 0)` → 재평가면 `(장부환율 재평가 @환율)`
- 자금정책 카드의 `원가승계 (권장)` → `(시스템 기본값)`. 회사 회계 실무가 정하는 것이지
  시스템이 권고할 사안이 아니다.

#### 사용자 조치 필요 (코드로 끝나지 않는 부분)
1. **자금정책 › FX 정책 › ② 정책 기준 › 외화 원장 회계정책**에서 `재평가` 선택.
   ⚠ **법인마다 따로 저장**해야 한다(전사 일괄 설정이 아니다).
2. ⚠ **이미 carryover 로 기록된 과거 대체 건은 자동으로 바뀌지 않는다** — 각 대체에 그때의
   `valuation_method` 가 박혀 있다(의도된 설계: 정책이 바뀌어도 과거 해석이 흔들리면 안 된다).
   9/17 재예치분처럼 회계 분개와 장부환율이 다른 건은 외화거래명세 › 데이터 등록 ›
   계좌 간 대체의 **대체 이력에서 원복 후 재등록**해야 한다.
   ⚠ 원복은 그 대체로 생긴 로트가 **아직 소진되지 않았을 때만** 서버가 허용한다.
   ⚠ 정기예금 해지로 만들어진 **이자 로트는 원복 대상이 아니다**(별도 `source_type='interest'`) —
   원복 후 남아 있으면 따로 정리할 것.

검증: tsc -b 0 errors · eslint 0 errors · vitest --dir src **169/169**(신규 8건 — 회계팀 분개
재현 포함). DB 마이그레이션 불필요.
