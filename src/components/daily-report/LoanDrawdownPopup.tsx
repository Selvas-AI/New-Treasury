...
export default function LoanDrawdownPopup({ company, onSaved, onClose }: Props) {
  ...
  async function handleSave() {
    ...
    if (!form.lender.trim()) { setError('금융기관명을 입력하세요'); return }
    if (amount <= 0) { setError('차입금액을 입력하세요'); return }
    if (!form.maturity) { setError('만기일을 입력하세요'); return }
    setSaving(true)
    setError(null)
    try {
      const newId = generateUUID()
      const { error: e } = await restInsert('loans', {
        id:         newId,
        company,
      ...
      const memo = form.memo.trim() || `${form.lender} 차입금 실행`
      await onSaved(amount, form.currency, memo, 'loan', newId)
      onClose()
    } catch (e) {
      setError(String(e))
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md mx-4 flex flex-col" style={{ animation: 'fadeInScale 0.18s ease-out both' }}>
        ...
        <div className="px-6 py-4 space-y-3">
          ...
          <div className="flex items-center gap-3">
            <span className={labelCls}>차입 종류</span>
          ...
          <div className="flex items-center gap-3">
            ...
            <input type="text" inputMode="numeric" value={form.amtStr}
              ...
              placeholder="차입금액 (원)" className={inputCls + ' text-right tabular-nums'} />
          </div>
          <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            <span className={labelCls}>실행일 <span className="text-red-400">*</span></span>
