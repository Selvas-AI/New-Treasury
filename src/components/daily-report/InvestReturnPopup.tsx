        ...
        <div className="px-6 pt-3 flex gap-2 shrink-0">
          {(['invest', 'equity'] as Tab[]).map(t => (
            <button key={t} onClick={() => { setTab(t); setSelected(null); setAmtStr('') }}
              className={`text-xs px-3 py-1.5 rounded-full font-medium transition-colors ${tab === t ? 'bg-blue-600 text-white' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-slate-700'}`}>
            ...
            <div className="space-y-1.5">
              {tab === 'invest' ? invests.map(inv => (
                <button key={inv.id} onClick={() => handleSelect(inv.id)}
                  className={`w-full text-left px-4 py-3 rounded-xl border transition-colors ${selectedId === inv.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-100 dark:border-slate-700 hover:border-blue-200 dark:hover:border-blue-700'}`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-xs font-semibold text-gray-800 dark:text-gray-100">{inv.bank}</span>
                      <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-gray-100 dark:bg-slate-700 text-gray-500">{inv.product}</span>
                    </div>
                ...
                <button key={eq.id} onClick={() => handleSelect(eq.id)}
                  className={`w-full text-left px-4 py-3 rounded-xl border transition-colors ${selectedId === eq.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-100 dark:border-slate-700 hover:border-blue-200 dark:hover:border-blue-700'}`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-xs font-semibold text-gray-800 dark:text-gray-100">{eq.name}</span>
                      <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-gray-100 dark:bg-slate-700 text-gray-500">{eq.market}</span>
                    </div>
          ...
          <div className="px-6 py-4 border-t border-gray-100 dark:border-slate-700 space-y-3 shrink-0">
            <div className="flex gap-2 items-center">
              ...
              <input type="text" inputMode="numeric" value={amtStr}
                ...
                placeholder="금액 (원)"
                className="flex-1 text-xs px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-right tabular-nums focus:outline-none focus:ring-1 focus:ring-blue-400" />
            </div>
            <div className="flex gap-2 items-center">
              ...
              <input type="text" value={memo} onChange={e => setMemo(e.target.value)} placeholder="메모 (선택)"
                className="flex-1 text-xs px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 focus:outline-none focus:ring-1 focus:ring-blue-400" />
            </div>
            {tab === 'invest' && (
