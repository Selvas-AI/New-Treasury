...
export default function FxLedgerPage() {
  ...
  const completed = trades.data.filter(row => row.status === '완료')
  ...
  return <div className="space-y-4 p-4 md:p-6">
    ...
    {tab === 'history' && <div className={CARD}><div className="mb-2 text-sm font-semibold">완료된 외화 매각</div>
      <div className="overflow-x-auto"><table className="w-full min-w-[780px] text-xs"><thead><tr className="border-b text-left text-gray-500"><th className="py-2">매각일</th><th>통화</th><th className="text-right">수량</th><th className="text-right">적용 원가</th><th className="text-right">매각환율</th><th className="text-right">실현손익</th><th>구분</th><th className="text-right">관리</th></tr></thead>
      <tbody>{completed.map(row => <tr key={row.id} className="border-b border-gray-100 dark:border-slate-800"><td className="py-2">{row.trade_date}</td><td>{row.currency}</td><td className="text-right">{row.amount_fx.toLocaleString()}</td><td className="text-right">{row.acq_rate?.toLocaleString() ?? '—'}</td><td className="text-right">{(row.completed_rate ?? row.trade_rate)?.toLocaleString() ?? '—'}</td><td className={`text-right ${(row.completed_pnl ?? row.fx_pnl ?? 0) >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{fmtKRW(row.completed_pnl ?? row.fx_pnl ?? 0)}</td><td>{row.memo?.includes('CSV 이관') ? '개시 전 실적' : '원장 연동'}</td><td className="text-right whitespace-nowrap">{canEdit() && row.memo?.includes('CSV 이관') && <button onClick={()=>setEditingTrade(row)} className="text-blue-600 hover:underline">수정</button>}{canDelete() && row.memo?.includes('CSV 이관') && <button onClick={()=>setPendingDelete({kind:'trade',row})} className="ml-2 text-red-600 hover:underline">삭제</button>}</td></tr>)}</tbody></table></div>
    </div>}
    {tab === 'pnl' && <>
