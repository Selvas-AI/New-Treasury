...
const STATUS: Record<ReportStatus, { label: string; cls: string }> = {
  draft:     { label: '작성 중',  cls: 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400' },
...
export default function DailyReportListPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900">
      ...
      {/* 헤더 */}
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-3 flex items-center gap-4 flex-wrap shrink-0">
            ...
            {companyNames.map(c => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full font-medium transition-colors ${
                  resolvedCompany === c ? 'bg-blue-600 text-white' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700'
                }`}
              >{c}</button>
            ))}
          </div>
          ...
          {[30, 60, 90].map(n => (
            <button
              ...
              className={`px-2 py-1 rounded-md font-medium transition-colors ${
                range === n ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/40 dark:text-blue-400' : 'hover:bg-gray-100 dark:hover:bg-gray-700'
              }`}
            >{n}일</button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
          ...
          {/* 요약 통계 */}
          <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center gap-6 text-xs">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-100 dark:border-gray-700">
                <th className="px-4 py-2.5 text-left font-medium text-gray-500 dark:text-gray-400 w-28">보고 대상일</th>
                <th className="px-4 py-2.5 text-left font-medium text-gray-500 dark:text-gray-400 w-24">상태</th>
                <th className="px-4 py-2.5 text-left font-medium text-gray-500 dark:text-gray-400">상신</th>
                <th className="px-4 py-2.5 text-left font-medium text-gray-500 dark:text-gray-400">승인</th>
                <th className="px-4 py-2.5 text-right font-medium text-gray-500 dark:text-gray-400 w-20">바로가기</th>
              </tr>
            </thead>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400 tabular-nums">
                      {row?.submitted_by
                        ? <><span className="font-medium text-gray-600 dark:text-gray-300">{row.submitted_by}</span> <span className="text-gray-400">{fmt(row.submitted_at)}</span></>
                        : '—'}
                    </td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400 tabular-nums">
                      {row?.approved_by
