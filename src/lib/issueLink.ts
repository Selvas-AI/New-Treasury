 ...
 *   fx_band_{법인}       → 외화거래명세 (정책 밴드 상한 초과 — 매도 발의 필요)
 *
...
export function issueSourceUrl(key: string, company: string | null | undefined): string | null {
  ...
  // 밴드 초과는 자금정책 메뉴 권한이 없는 실무자도 조치해야 하므로 외화거래명세로 보낸다
  // (거기 매도 발의 카드가 있다 — 세션26차 11일차).
  if (key.startsWith('fx_band_'))   return `/fx-ledger/${c}?tab=orders`
...
export function issueSourceLabel(key: string): string {
  ...
  if (key.startsWith('fx_band_'))   return '매도 발의하기'
